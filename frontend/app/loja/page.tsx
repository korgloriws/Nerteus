...
export default async function LojaPage() {
  ...
  return (
    <main className="space-y-6 sm:space-y-8 text-foreground min-w-0">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-xs uppercase tracking-[0.3em] text-muted">Afiliados</p>
          <h1 className="fluid-h1 font-bold">Loja</h1>
      ...
      {products.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card/40 p-6 sm:p-10 text-center text-muted">
          Nenhum produto ativo no momento. Volte em breve.
        </div>
      ) : (
        <div className="grid items-stretch gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
